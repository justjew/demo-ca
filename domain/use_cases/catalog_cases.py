...
class ManageStopListUseCase:
    ...
    def add_product_to_stop_list(self, outlet_id: uuid.UUID, product_id: uuid.UUID) -> None:
    ...
    def remove_product_from_stop_list(self, outlet_id: uuid.UUID, product_id: uuid.UUID) -> None:
        outlet = self.outlet_repo.get_by_id(outlet_id)
        if not outlet:
            return
